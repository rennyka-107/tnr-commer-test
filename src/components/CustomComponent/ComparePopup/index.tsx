    ...
    <CardStyled hidden={comparePopUpItem.length === 0}>
      ...
      >
        {comparePopUpItem.map(item => (
          <PopUpItem data={item} onRemove={remove(item.productId)} key={item.productId}/>
        ))}
        {Array.from({ length: 3 - comparePopUpItem.length}).map((item, index) => (
          <ItemWrapper key={index}/>
        ))}
        <Stack direction="column" justifyContent="center" spacing={2}>
          <Button disabled={comparePopUpItem.length < 2} variant={'contained'} onClick={onCompare}>
